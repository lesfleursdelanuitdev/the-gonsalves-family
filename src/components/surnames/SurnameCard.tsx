...
import { CaseSensitive, TrendingDown, TrendingUp, UsersRound } from "lucide-react";
import type { SurnameRankTier } from "@/lib/surnames/surname-list-helpers";
import type { PublicSurname } from "./types";

export function SurnameCard({
  surname,
  rankTier,
}: {
  surname: PublicSurname;
  rankTier?: SurnameRankTier | null;
}) {
  return (
      ...
      <div className="min-w-0 space-y-4 p-4 sm:p-5">
        <div className="flex items-center justify-center gap-2 border-y border-border-subtle/70 py-3">
          ...
          <div className="text-center">
            ...
            <p className="mt-0.5 text-sm font-medium text-heading">{surname.peopleCount.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-col gap-2">
          ...
          >
            Browse people with this surname <span aria-hidden>&rarr;</span>
          </Link>
          <Link
