...
interface AlbumPickerProps {
  value: AlbumOption | null;
  onChange: (album: AlbumOption | null) => void;
  placeholder?: string;
  className?: string;
}

export function AlbumPicker({ value, onChange, placeholder = "Search albums…", className }: AlbumPickerProps) {
  const [query, setQuery] = useState("");
  const [suggestions, setSuggestions] = useState<AlbumOption[]>([]);
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const url = `/api/tree/advanced-search/albums?q=${encodeURIComponent(q)}`;
  ...
  const select = useCallback((album: AlbumOption) => {
    onChange(album);
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (!(e.target as Element).closest("[data-album-picker]")) setOpen(false);
    };
  ...
  return (
    <div className={cn("relative", className)} data-album-picker>
      ...
      {open && (
        <ul role="listbox" className="absolute z-50 mt-1 w-full overflow-hidden rounded-xl border border-[#e0d8cc] bg-white shadow-[0_8px_24px_rgba(60,45,25,0.12)]">
          {suggestions.length === 0 ? (
            <li className="px-4 py-3 font-body text-sm text-muted">{loading ? "Loading…" : "No albums found."}</li>
          ) : (
            suggestions.map((album, i) => (
              <li key={album.id} role="option" aria-selected={i === activeIndex}
                onMouseDown={() => select(album)} onMouseEnter={() => setActiveIndex(i)}
                className={cn(
                ...
                <span className="font-body text-sm text-text">{album.name}</span>
              </li>
            ))
          )}
        </ul>
