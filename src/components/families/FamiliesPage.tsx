...
import { filterPublicFamilies } from "@/lib/families/filter-public-families";
import { FamilyCard } from "./FamilyCard";
import {
  buildFamiliesFilterButtonLabel,
  EMPTY_FAMILIES_FILTERS,
  FamiliesActiveFilterChips,
  FamiliesFilterPanel,
  type FamiliesFilterState,
} from "./FamiliesFilterPanel";
import type { PublicFamily } from "./types";

type SortMode = "az" | "za" | "mostChildren" | "marriageNewest" | "marriageOldest";
...
const SORT_OPTIONS: { value: SortMode; label: string }[] = [
  { value: "az", label: "A-Z" },
  { value: "za", label: "Z-A" },
  { value: "mostChildren", label: "Most children" },
  { value: "marriageNewest", label: "Marriage (newest)" },
  { value: "marriageOldest", label: "Marriage (oldest)" },
];
...
function FamiliesMobileControls({
  searchQuery,
  onSearchQueryChange,
  filterLabel,
  filterMenuOpen,
  ...
  const filterIsActive = filterLabel !== "Filter families";
      ...
      >
        <div className={`${expanded ? "mb-1.5" : ""} flex items-center justify-between gap-3`}>
          <button
            ...
            aria-controls="families-mobile-controls-panel"
        ...
        <div id="families-mobile-controls-panel" className={expanded ? "flex items-center gap-1.5" : "hidden"}>
          ...
          <button
            ...
            className="relative inline-flex h-10 w-11 shrink-0 items-center justify-center rounded-2xl border border-border-subtle bg-[rgba(255,250,240,0.58)] text-link shadow-[inset_0_1px_0_rgba(255,255,255,0.55)] transition hover:bg-link-soft-bg hover:text-link-soft-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
          >
          ...
          <label className="relative block w-28 shrink-0">
            ...
            <select
              ...
              className="h-10 w-full cursor-pointer rounded-2xl border border-border-subtle bg-[rgba(255,250,240,0.58)] px-2 text-sm font-semibold text-heading shadow-[inset_0_1px_0_rgba(255,255,255,0.55)] outline-none transition focus-visible:ring-2 focus-visible:ring-focus-ring"
            >
...
export function FamiliesPage({ families }: { families: PublicFamily[] }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [filters, setFilters] = useState<FamiliesFilterState>(EMPTY_FAMILIES_FILTERS);
  const [draftFilters, setDraftFilters] = useState<FamiliesFilterState>(EMPTY_FAMILIES_FILTERS);
  ...
  const filtered = useMemo(() => {
    ...
    const byFilters = filterPublicFamilies(families, filters).filter((family) => {
      if (!q) return true;
      const haystack = [
        family.title,
        family.marriagePlaceLabel ?? "",
        ...family.partners.map((partner) => partner.fullName),
      ]
        .join(" ")
        .toLowerCase();
    ...
    sorted.sort((a, b) => {
      ...
      if (sortMode === "mostChildren") return b.childrenCount - a.childrenCount || a.title.localeCompare(b.title);
      if (sortMode === "marriageNewest") {
        return (b.marriageYear ?? -9999) - (a.marriageYear ?? -9999) || a.title.localeCompare(b.title);
      }
      return (a.marriageYear ?? 9999) - (b.marriageYear ?? 9999) || a.title.localeCompare(b.title);
    });
    return sorted;
  }, [families, searchQuery, filters, sortMode]);
  ...
  const clearFilters = () => {
    setDraftFilters(EMPTY_FAMILIES_FILTERS);
    setFilters(EMPTY_FAMILIES_FILTERS);
    setPage(1);
  };
  ...
  return (
    <div className="flex min-h-screen min-w-0 max-w-full flex-col overflow-x-hidden bg-bg pb-32 text-text sm:pb-0">
              ...
              <div className="grid min-w-0 max-w-full gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:items-end lg:gap-10">
                <div className="min-w-0 max-w-full space-y-5 p-5 backdrop-blur-md [-webkit-backdrop-filter:blur(14px)] [backdrop-filter:blur(14px)] sm:p-6">
                  <nav aria-label="Breadcrumb" className="flex min-w-0 flex-wrap items-center gap-2 text-xs tracking-[0.06em] text-muted">
                    ...
                    <span className="shrink-0 text-subtle">/</span>
                    <span className="min-w-0 text-heading">Families</span>
                  </nav>
                  ...
                  <p className="max-w-2xl text-base leading-relaxed text-muted sm:text-lg md:text-xl">
                    Browse marriages and family groups in our tree. Each card shows the couple, their children, marriage
                    date, and whether a divorce is recorded.
                  </p>
                </div>
            ...
            <div className="min-w-0 max-w-full space-y-5">
              <div className="min-w-0 max-w-full rounded-2xl border border-border/80 bg-surface/90 p-4 shadow-[0_10px_26px_rgba(60,45,25,0.06)] sm:p-5">
                <div className="grid min-w-0 max-w-full gap-3 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center">
                  <p className="text-sm font-semibold text-heading">{filtered.length} Families</p>
                  ...
                  <div className="hidden flex-wrap items-center gap-2 rounded-xl border border-border/40 bg-black/[0.03] p-1.5 sm:flex">
                    <div className="relative min-w-0">
                      ...
                      >
                        ...
                        <span className="min-w-0 truncate">{buildFamiliesFilterButtonLabel(filters)}</span>
                      </button>
                    ...
                    <label className="font-body flex min-w-0 items-center gap-2 text-sm text-muted">
                      ...
                      <select
                        ...
                        className="h-9 min-w-[10rem] max-w-full cursor-pointer rounded-lg border border-border bg-surface px-2.5 py-1 text-sm font-medium text-text outline-none focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-2 focus-visible:ring-offset-bg"
                      >
                        {SORT_OPTIONS.map((option) => (
                          <option key={option.value} value={option.value}>
                            {option.label}
              ...
              <div className="grid min-w-0 max-w-full grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                {pageItems.length === 0 ? (
                  <div className="col-span-full rounded-2xl border border-dashed border-border-subtle/80 bg-surface/60 px-4 py-12 text-center text-sm text-muted">
                    No families match this search or filters.
                  </div>
                ) : (
                  pageItems.map((family) => <FamilyCard key={family.id} family={family} />)
                )}
              </div>
      ...
      {filterMenuOpen && isNarrow && typeof document !== "undefined"
        ...
        : null}
      <FamiliesMobileControls
        searchQuery={searchQuery}
        onSearchQueryChange={(value) => {
          setSearchQuery(value);
          setPage(1);
        }}
        filterLabel={buildFamiliesFilterButtonLabel(filters)}
