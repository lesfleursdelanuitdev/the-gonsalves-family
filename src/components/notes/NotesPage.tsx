...
import { filterNotes, hasActiveNotesFilters } from "@/lib/notes/filter-notes";
import type { PublicNote } from "@/lib/notes/public-note-types";
import {
  buildNotesFilterButtonLabel,
  EMPTY_NOTES_FILTERS,
  NotesActiveFilterChips,
  NotesFilterPanel,
  type NotesFilterState,
} from "./NotesFilterPanel";
...
function sortKey(note: PublicNote): string {
  return (note.xref ?? note.displayNumber).toLowerCase();
}

export function NotesPage({ notes }: { notes: PublicNote[] }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [filters, setFilters] = useState<NotesFilterState>(EMPTY_NOTES_FILTERS);
  const [draftFilters, setDraftFilters] = useState<NotesFilterState>(EMPTY_NOTES_FILTERS);
  ...
  const clearFilters = () => {
    setDraftFilters(EMPTY_NOTES_FILTERS);
  ...
  const filtered = useMemo(() => {
    ...
    const byFilters = filterNotes(notes, filters);
    const matched = q ? byFilters.filter((note) => note.searchText.includes(q)) : byFilters;
    ...
    sorted.sort((a, b) => {
      ...
      if (sortMode === "az") return sortKey(a).localeCompare(sortKey(b));
      return sortKey(b).localeCompare(sortKey(a));
    });
    return sorted;
  }, [notes, searchQuery, filters, sortMode]);
  ...
  return (
    <div className="flex min-h-screen min-w-0 max-w-full flex-col overflow-x-hidden bg-bg pb-32 text-text sm:pb-0">
              ...
              <div className="grid min-w-0 max-w-full gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:items-end lg:gap-10">
                <div className="min-w-0 max-w-full space-y-5 p-5 backdrop-blur-md [-webkit-backdrop-filter:blur(14px)] [backdrop-filter:blur(14px)] sm:p-6">
                  <nav aria-label="Breadcrumb" className="flex min-w-0 flex-wrap items-center gap-2 text-xs tracking-[0.06em] text-muted">
                    ...
                    <Link href="/archive/documents" className="min-w-0 shrink transition hover:text-link">
                      Archive
                    </Link>
                    <span className="shrink-0 text-subtle">/</span>
                    <span className="min-w-0 text-heading">Notes</span>
                  </nav>
                  ...
                  <p className="max-w-2xl text-base leading-relaxed text-muted sm:text-lg md:text-xl">
                    Research remarks and transcribed notes from our family tree, linked to people, families, events, and
                    sources.
                  </p>
                </div>
        ...
        <Section noPadding className="min-w-0 overflow-x-hidden pb-6 pt-2 sm:py-8 md:py-10">
          <PageContainer narrow>
            <div className="min-w-0 max-w-full space-y-5">
                  ...
                  <div className="hidden flex-wrap items-center gap-2 rounded-xl border border-border/40 bg-black/[0.03] p-1.5 sm:flex">
                    <div className="relative min-w-0">
                      ...
                      >
                        ...
                        <span className="min-w-0 truncate">{buildNotesFilterButtonLabel(filters)}</span>
                      </button>
              ...
              <div className="grid min-w-0 max-w-full grid-cols-1 gap-5 sm:notes-card-grid sm:grid-cols-2 sm:gap-6 xl:grid-cols-3">
                {pageItems.length === 0 ? (
                  <div className="col-span-full rounded-2xl border border-dashed border-border-subtle/80 bg-surface/60 px-4 py-12 text-center text-sm text-muted">
                    {notes.length === 0
                      ? "No notes are in the tree yet."
                      : hasActiveNotesFilters(filters) || searchQuery.trim()
                        ? "No notes match this search or filter."
                        : "No notes match this search."}
                  </div>
                ) : (
                  pageItems.map((note) => <NoteCard key={note.id} note={note} />)
                )}
              </div>
      ...
      <ListPageMobileControls
        ...
        panelId="notes-mobile-controls-panel"
        filterDefaultLabel="Filter notes"
        ...
        filterLabel={buildNotesFilterButtonLabel(filters)}
