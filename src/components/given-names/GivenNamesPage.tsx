...
import {
  ...
  filterGivenNames,
  sortGivenNames,
  type GivenNameListSortMode,
} from "@/lib/given-names/given-name-list-helpers";
import {
  buildGivenNamesFilterButtonLabel,
  EMPTY_GIVEN_NAMES_FILTERS,
  GivenNamesActiveFilterChips,
  GivenNamesFilterPanel,
  type GivenNamesFilterState,
} from "./GivenNamesFilterPanel";
import { GivenNameCard } from "./GivenNameCard";
import type { PublicGivenName } from "./types";

const PAGE_SIZE = 12;

const SORT_OPTIONS: { value: GivenNameListSortMode; label: string }[] = [
  { value: "az", label: "A-Z" },
  { value: "za", label: "Z-A" },
  { value: "mostCommon", label: "Most common" },
  { value: "leastCommon", label: "Least common" },
];
...
export function GivenNamesPage({ givenNames }: { givenNames: PublicGivenName[] }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [filters, setFilters] = useState<GivenNamesFilterState>(EMPTY_GIVEN_NAMES_FILTERS);
  const [draftFilters, setDraftFilters] = useState<GivenNamesFilterState>(EMPTY_GIVEN_NAMES_FILTERS);
  const [filterMenuOpen, setFilterMenuOpen] = useState(false);
  const [sortMode, setSortMode] = useState<GivenNameListSortMode>("az");
  const [page, setPage] = useState(1);
  const [isNarrow, setIsNarrow] = useState(false);

  const rankTierById = useMemo(() => computeGivenNameRankTierById(givenNames), [givenNames]);
  const availableLetters = useMemo(() => availableGivenNameLetters(givenNames), [givenNames]);
  ...
  const filtered = useMemo(() => {
    const matched = filterGivenNames(givenNames, {
      searchQuery,
      filters,
      rankTierById,
    });
    return sortGivenNames(matched, sortMode);
  }, [givenNames, searchQuery, filters, rankTierById, sortMode]);
  ...
  const clearFilters = () => {
    setDraftFilters(EMPTY_GIVEN_NAMES_FILTERS);
    setFilters(EMPTY_GIVEN_NAMES_FILTERS);
    setPage(1);
  };
  ...
  return (
    <div className="flex min-h-screen min-w-0 max-w-full flex-col overflow-x-hidden bg-bg pb-32 text-text sm:pb-0">
              ...
              <div className="grid min-w-0 max-w-full gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:items-end lg:gap-10">
                <div className="min-w-0 max-w-full space-y-5 p-5 backdrop-blur-md [-webkit-backdrop-filter:blur(14px)] [backdrop-filter:blur(14px)] sm:p-6">
                  <nav aria-label="Breadcrumb" className="flex min-w-0 flex-wrap items-center gap-2 text-xs tracking-[0.06em] text-muted">
                    ...
                    <span className="shrink-0 text-subtle">/</span>
                    <span className="min-w-0 text-heading">Given Names</span>
                  </nav>
                  ...
                  <p className="max-w-2xl text-base leading-relaxed text-muted sm:text-lg md:text-xl">
                    Browse family given names recorded in our tree. Use filters to narrow by letter or frequency rank,
                    then open a profile for statistics and people who carry the name.
                  </p>
                </div>
            ...
            <div className="min-w-0 max-w-full space-y-5">
              <div className="min-w-0 max-w-full rounded-2xl border border-border/80 bg-surface/90 p-4 shadow-[0_10px_26px_rgba(60,45,25,0.06)] sm:p-5">
                <div className="grid min-w-0 max-w-full gap-3 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center">
                  <p className="text-sm font-semibold text-heading">{filtered.length} Given Names</p>
                  ...
                  <label className="relative hidden min-w-0 sm:block">
                    ...
                    <input
                      ...
                      placeholder="Search given names..."
                  ...
                  <div className="hidden flex-wrap items-center gap-2 rounded-xl border border-border/40 bg-black/[0.03] p-1.5 sm:flex">
                    ...
                    <label className="font-body flex min-w-0 items-center gap-2 text-sm text-muted">
                      ...
                      <select
                        aria-label="Sort given names"
                        value={sortMode}
                        onChange={(e) => {
                          setSortMode(e.target.value as GivenNameListSortMode);
                          setPage(1);
                        }}
              ...
              <div className="grid min-w-0 max-w-full grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                {pageItems.length === 0 ? (
                  <div className="col-span-full rounded-2xl border border-dashed border-border-subtle/80 bg-surface/60 px-4 py-12 text-center text-sm text-muted">
                    No given names match this search or filters.
                  </div>
                ) : (
                  pageItems.map((givenName) => (
                    <GivenNameCard
                      key={givenName.id}
                      givenName={givenName}
                      rankTier={rankTierById.get(givenName.id) ?? null}
                    />
                  ))
                )}
              </div>
      ...
      <ListPageMobileControls
        entityName="Given Names"
        searchPlaceholder="Search given names..."
        panelId="given-names-mobile-controls-panel"
        filterDefaultLabel="Filter given names"
        sortAriaLabel="Sort given names"
        searchQuery={searchQuery}
        onSearchQueryChange={(value) => {
          setSearchQuery(value);
          setPage(1);
        }}
        filterLabel={buildGivenNamesFilterButtonLabel(filters)}
