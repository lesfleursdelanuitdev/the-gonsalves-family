...
import {
  GIVEN_NAME_LETTER_OTHER,
  type GivenNameRankFilter,
} from "@/lib/given-names/given-name-list-helpers";

export type GivenNamesFilterState = {
  letter: string | null;
  rankFilter: GivenNameRankFilter;
};

type GivenNamesFilterSection = "letter" | "frequency";

export const EMPTY_GIVEN_NAMES_FILTERS: GivenNamesFilterState = {
  letter: null,
  rankFilter: "all",
};
...
function withFilter(
  filters: GivenNamesFilterState,
  patch: Partial<GivenNamesFilterState>,
): GivenNamesFilterState {
  return { ...filters, ...patch };
}

export function hasActiveGivenNamesFilters(filters: GivenNamesFilterState): boolean {
  return filters.letter != null || filters.rankFilter !== "all";
}

function activeFilterCount(filters: GivenNamesFilterState): number {
  let count = 0;
...
export function buildGivenNamesFilterButtonLabel(filters: GivenNamesFilterState): string {
  const count = activeFilterCount(filters);
  return count === 0 ? "Filter given names" : `${count} filter${count === 1 ? "" : "s"} active`;
}

function letterSummary(filters: GivenNamesFilterState): string {
  if (filters.letter == null) return "Any starting letter";
  if (filters.letter === GIVEN_NAME_LETTER_OTHER) return "Starts with #";
  return `Starts with ${filters.letter}`;
}

function frequencySummary(filters: GivenNamesFilterState): string {
...
function sectionSummary(section: GivenNamesFilterSection, filters: GivenNamesFilterState): string {
...
function ActiveGivenNamesFilterChips({
  filters,
  onChange,
  compact,
}: {
  filters: GivenNamesFilterState;
  onChange: (filters: GivenNamesFilterState) => void;
  compact?: boolean;
}) {
    ...
    chips.push(
      <span key="letter" className={chipClass}>
        <span className="min-w-0 truncate">
          Letter: {filters.letter === GIVEN_NAME_LETTER_OTHER ? "#" : filters.letter}
        </span>
        <button
          type="button"
  ...
  if (filters.rankFilter !== "all") {
    chips.push(
      <span key="rank" className={chipClass}>
        <span className="min-w-0 truncate">
          {filters.rankFilter === "top10" ? "Top 10 givenNames" : "Bottom 10 givenNames"}
        </span>
        <button
          type="button"
          className="shrink-0 rounded-full p-0.5 text-muted hover:bg-black/[0.06] hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
...
export function GivenNamesActiveFilterChips({
  filters,
  onChange,
}: {
  filters: GivenNamesFilterState;
  ...
  if (!hasActiveGivenNamesFilters(filters)) return null;
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2">
      <ActiveGivenNamesFilterChips filters={filters} onChange={onChange} compact />
    </div>
  );
}
...
function LetterSectionDetail({
  ...
  return (
    <div className="space-y-4">
      <p className="font-body text-sm text-muted">Show givenNames whose display name begins with a specific letter.</p>
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5" role="group" aria-label="Starting letter">
          ...
          {availableLetters.map((l) => (
            <button
              ...
              aria-label={l === GIVEN_NAME_LETTER_OTHER ? "Starts with number or symbol" : `Starts with ${l}`}
            >
              {l === GIVEN_NAME_LETTER_OTHER ? "#" : l}
            </button>
          ))}
        </div>
      </div>
...
function SectionDetail({
  ...
  filters: GivenNamesFilterState;
  activeSection: GivenNamesFilterSection;
  availableLetters: string[];
  onChange: (filters: GivenNamesFilterState) => void;
}) {
  ...
  return (
    <ChoiceGroup
      ...
      options={[
        { value: "all", label: "All given names" },
...
function LeftNavColumn({
  ...
  filters: GivenNamesFilterState;
  activeSection: GivenNamesFilterSection;
  onSelectSection: (section: GivenNamesFilterSection) => void;
  onClearFilters: () => void;
...
export function GivenNamesFilterPanel({
  variant = "panel",
  filters,
  availableLetters,
  onChange,
  ...
  const [activeSection, setActiveSection] = useState<GivenNamesFilterSection>("letter");
  const [mobileStack, setMobileStack] = useState<"main" | GivenNamesFilterSection>("main");

  const mobileTitles: Record<GivenNamesFilterSection, string> = {
    letter: "Starting letter",
    frequency: "Frequency rank",
  };

  const openMobileSection = useCallback((section: GivenNamesFilterSection) => {
    ...
    return (
      <div className="flex min-h-0 flex-1 flex-col bg-[#f5f1ea]">
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#e8e0d4] px-4 py-4">
          <div>
            <h2 className="font-heading text-lg font-semibold tracking-tight text-heading">Filter given names</h2>
  ...
  return (
    <div className="flex max-h-[min(90vh,720px)] min-h-[320px] flex-col overflow-hidden rounded-xl border border-[#e8e0d4] bg-white shadow-[0_16px_48px_-12px_rgba(55,40,28,0.18)]">
      ...
      <div className="shrink-0 border-b border-[#ebe4d9] px-5 py-2">
        <ActiveGivenNamesFilterChips filters={filters} onChange={onChange} />
      </div>
