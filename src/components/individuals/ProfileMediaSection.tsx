...
import { INDIVIDUAL_MEDIA_PEEK_SAMPLE_LIMIT, useIndividualMediaPeek } from "@/components/TreeViewer/v2/PersonDetailOverlay/hooks/useIndividualMediaPeek";
  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-[linear-gradient(180deg,rgba(129,89,58,0.12),rgba(129,89,58,0.05))] sm:aspect-[16/11]">
        {src ? (
          // eslint-disable-next-line @next/next/no-img-element -- GEDCOM media may be served from the admin uploads origin.
          <img
            src={src}
            alt=""
      ...
      <p className="hidden min-w-0 truncate p-4 font-heading text-base font-medium text-heading sm:block">{label}</p>
    </Link>
  );
}
...
export function ProfileMediaSection({
  ...
  const mediaPeek = useIndividualMediaPeek(xref);
  const peek = mediaPeek.data;
  const albumHref = `/media/album-view?kind=generated&type=individual&id=${encodeURIComponent(individualId)}`;
  const loading = mediaPeek.status === "loading";
  const failed = mediaPeek.status === "error";
  const showRandomize =
    peek != null &&
    peek.totalCount > INDIVIDUAL_MEDIA_PEEK_SAMPLE_LIMIT &&
    peek.samples.length > 0;
        ...
        <div>
          ...
          {peek?.totalCount ? (
            <p className="mt-2 text-sm leading-relaxed text-muted">
              Showing {peek.samples.length} of {peek.totalCount} linked media items. Randomize the preview or open the generated album to browse everything.
            </p>
          ) : null}
        </div>
