...
export async function GET(
  ...
  try {
    ...
    const spouseFamilyByKey = new Map<
      string,
      {
        ...
        spouse: {
          name: string | null;
          xref: string;
          death?: { date: string | null; place: string | null; year?: number | null; month?: number | null; day?: number | null };
        };
        children: {
          name: string | null;
          xref: string;
    ...
    for (const r of famSpouseRows) {
      ...
      if (!spouseFamilyByKey.has(fid)) {
        ...
        spouseFamilyByKey.set(fid, {
          ...
          spouse: {
            name: stripSlashesFromName(r.spouse_name as string) ?? null,
      ...
      if (r.child_id && !entry.children.some((c) => c.xref === (r.child_xref as string))) {
        ...
        const hasDeath = r.child_death_date != null || r.child_death_place != null || r.child_death_year != null || r.child_death_month != null || r.child_death_day != null;
        entry.children.push({
          id: (r.child_id as string) ?? "",
          name: stripSlashesFromName(r.child_name as string) ?? null,
          xref: (r.child_xref as string) ?? "",
          birth: hasBirth ? { date: (r.child_birth_date as string) ?? null, place: (r.child_birth_place as string) ?? null, year: r.child_birth_year != null ? Number(r.child_birth_year) : null, month: r.child_birth_month != null ? Number(r.child_birth_month) : null, day: r.child_birth_day != null ? Number(r.child_birth_day) : null } : undefined,
          death: hasDeath ? { date: (r.child_death_date as string) ?? null, place: (r.child_death_place as string) ?? null, year: r.child_death_year != null ? Number(r.child_death_year) : null, month: r.child_death_month != null ? Number(r.child_death_month) : null, day: r.child_death_day != null ? Number(r.child_death_day) : null } : undefined,
        });
      }
    }
    ...
    for (const r of familyEventRows) {
      ...
      events.push(eventItem(r, "family", { familyId, spouseName: spouse?.name ?? null, spouseXref: spouse?.xref ?? "" }));
    }
    for (const fam of familiesAsSpouse) {
      if (fam.spouse.death?.date || fam.spouse.death?.place || fam.spouse.death?.year != null || fam.spouse.death?.month != null || fam.spouse.death?.day != null) {
        events.push(
          eventItem(
            {
              event_type: "DEAT",
            ...
            "spouseDeath",
            { familyId: fam.family.id, spouseName: fam.spouse.name ?? null, spouseXref: fam.spouse.xref }
          )
        );
      }
    ...
    for (const r of childMarriageRows) {
      ...
      const childName = stripSlashesFromName((isChildHusband ? r.husband_name : r.wife_name) as string) ?? null;
      const spouseXref = (isChildHusband ? r.wife_xref : r.husband_xref) as string;
      const spouseName = stripSlashesFromName((isChildHusband ? r.wife_name : r.husband_name) as string) ?? null;
      events.push(
        eventItem(
          {
            event_type: "MARR",
          ...
          "childMarriage",
          { familyId: r.family_id as string, childXref, childName, spouseXref, spouseName }
        )
      );
    }
    ...
    const grandchildBirthRows: Row[] =
      childIds.length > 0
        ? await prisma.$queryRaw<Row[]>(
            Prisma.sql`
              SELECT f.id AS family_id, ch.xref AS grandchild_xref, ch.full_name AS grandchild_name,
        ...
        events.push(
          eventItem(
            ...
            "grandchildBirth",
            {
              ...
              childName: stripSlashesFromName(r.grandchild_name as string) ?? null,
            }
          )
        );
      }
    ...
    if (originFamilyIds.length > 0) {
      const parentDeathRows = await prisma.$queryRaw<Row[]>(
        Prisma.sql`
          SELECT f.id AS family_id,
                 husb.xref AS father_xref, husb.full_name AS father_name,
                 ...
                 wife.xref AS mother_xref, wife.full_name AS mother_name,
        ...
        if (hasFatherDeath) {
          events.push(
            eventItem(
              ...
              {
                familyId: r.family_id as string,
                spouseName: stripSlashesFromName(r.father_name as string) ?? null,
        ...
        if (hasMotherDeath) {
          events.push(
            eventItem(
              ...
              {
                familyId: r.family_id as string,
                spouseName: stripSlashesFromName(r.mother_name as string) ?? null,
      ...
      const siblingDeathRows = await prisma.$queryRaw<Row[]>(
        Prisma.sql`
          SELECT fch.family_id, ch.xref AS sibling_xref, ch.full_name AS sibling_name,
          ...
          events.push(
            eventItem(
              ...
              "siblingDeath",
              {
                ...
                childName: stripSlashesFromName(r.sibling_name as string) ?? null,
              }
            )
          );
        }
      ...
      // Collect unique parent IDs (for grandparent lookup)
      ...
      if (parentIds.length > 0) {
        ...
        if (grandparentFamilyIds.length > 0) {
          const grandparentDeathRows = await prisma.$queryRaw<Row[]>(
            Prisma.sql`
              SELECT f.id AS family_id,
                     husb.xref AS grandfather_xref, husb.full_name AS grandfather_name,
                     ...
                     wife.xref AS grandmother_xref, wife.full_name AS grandmother_name,
            ...
            if (hasGrandfatherDeath) {
              events.push(
                eventItem(
                  ...
                  {
                    familyId: r.family_id as string,
                    spouseName: stripSlashesFromName(r.grandfather_name as string) ?? null,
            ...
            if (hasGrandmotherDeath) {
              events.push(
                eventItem(
                  ...
                  {
                    familyId: r.family_id as string,
                    spouseName: stripSlashesFromName(r.grandmother_name as string) ?? null,
