...
import { guessContentType, resolveSiteMediaDiskPath } from "@/lib/uploadsServe";

/**
 * Serves public site media from disk for `/uploads/site-media/...`.
...
export async function GET(_req: Request, ctx: { params: Promise<{ path?: string | string[] }> }) {
  ...
  const diskPath = resolveSiteMediaDiskPath(normalizeSegments(raw));
  if (!diskPath) {
