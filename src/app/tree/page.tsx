...
export const metadata = {
  title: "Explore the Tree · The Gonsalves Family",
  description:
    "Explore the family tree — the interactive viewer, individuals, families, surnames, given names, events, and places.",
};

const HERO_DESKTOP_PHOTO = "/images/albumsCoverImage.png";
const HERO_MOBILE_PHOTO = "/images/albumsCoverImageMobile.png";
const EXPLORE_BAND_PHOTO = "/images/family/gonsalves-family.jpg";

/** Lucide icon for each Tree nav item, keyed by its navConfig icon key. */
const TREE_ICONS: Record<string, typeof GitBranch> = {
...
/** Section cards are driven from the Tree menu group so the hub stays in sync with the nav. */
const TREE_SECTIONS = SITE_NAV_GROUPS.find((g) => g.id === "tree")?.items ?? [];

/** Spotlight people — Augustinho Thomas Gonsalves, Norman Peter Gonsalves, Mary Mias Gracis. */
const SPOTLIGHT_IDS = [
  "8450ef75-63c1-4a84-a1ee-0c5a9ff16f30",
  "ce85c538-0185-469d-87d4-edb4514d1ad6",
  "7f01d81a-3107-4700-a2c4-89c2b2435ee7",
];

function SpotlightHeading({ title, seeAllHref, seeAllLabel }: {
  title: string;
  seeAllHref: string;
  seeAllLabel: string;
}) {
...
export default async function ExploreTreePage() {
  let people: Awaited<ReturnType<typeof loadPublicIndividualsByIds>> = [];
  let families: Awaited<ReturnType<typeof loadPublicFamilies>> = [];
  let places: Awaited<ReturnType<typeof loadPublicPlaces>> = [];
  try {
    const [loadedPeople, loadedFamilies, loadedPlaces] = await Promise.all([
      loadPublicIndividualsByIds(SPOTLIGHT_IDS),
      loadPublicFamilies(),
      loadPublicPlaces(),
    ]);
    // loadPublicIndividualsByIds does not preserve input order — restore it.
    people = SPOTLIGHT_IDS.map((id) => loadedPeople.find((p) => p.id === id)).filter(
      (p): p is NonNullable<typeof p> => Boolean(p),
    );
    families = loadedFamilies.slice(0, 3);
    places = loadedPlaces.slice(0, 6);
  } catch {
    people = [];
    families = [];
    places = [];
  ...
  return (
    <div className="flex min-h-screen min-w-0 max-w-full flex-col overflow-x-hidden bg-bg pb-32 text-text sm:pb-0">
      ...
      <main className="min-w-0 flex-1 overflow-x-hidden">
            ...
            <PageContainer narrow>
              <div className="min-w-0 max-w-full space-y-5 p-5 backdrop-blur-md [-webkit-backdrop-filter:blur(14px)] [backdrop-filter:blur(14px)] sm:p-6">
                <nav aria-label="Breadcrumb" className="flex min-w-0 flex-wrap items-center gap-2 text-xs tracking-[0.06em] text-muted">
                  ...
                  <span className="min-w-0 text-heading">Family Tree</span>
                </nav>

                <p className="section-subtitle">Explore the Tree</p>
                ...
                <h1 className="break-words font-heading text-4xl font-semibold leading-[1.05] tracking-tight text-heading sm:text-5xl md:text-6xl">
                  Explore the <span className="italic">family tree</span>
                </h1>
                ...
                <p className="max-w-2xl text-base leading-relaxed text-muted sm:text-lg md:text-xl">
                  Walk the family tree — the interactive viewer, the people and families in it, and the
                  names, events, and places that connect them.
                </p>
        ...
        {/* Start here — sepia feature band into the Tree Viewer. */}
            ...
            >
              ...
              <div className="relative z-[2] max-w-[600px] px-[26px] py-[28px] sm:px-[42px] sm:py-[34px]">
                ...
                <p className="mt-[9px] max-w-[46ch] font-body text-[15px] leading-[1.6] text-[rgba(247,239,220,0.82)]">
                  Six generations, one living map. Pan and zoom through every branch in the Tree Viewer.
                </p>
        ...
        {/* Section grid — one card per Tree menu item */}
        <Section noPadding className="min-w-0 overflow-x-hidden pb-8 md:pb-10">
          <PageContainer narrow>
            <div className="grid min-w-0 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
              {TREE_SECTIONS.map((item) => {
                const Icon = TREE_ICONS[item.icon] ?? GitBranch;
                return (
                  <Link
                    key={item.href}
                    href={item.href}
        ...
        {people.length > 0 && (
          <Section noPadding className="min-w-0 overflow-x-hidden pb-10 md:pb-12">
            <PageContainer narrow>
              <SpotlightHeading title="People to meet" seeAllHref="/individuals" seeAllLabel="See all people" />
        ...
        {families.length > 0 && (
          <Section noPadding className="min-w-0 overflow-x-hidden pb-10 md:pb-12">
            <PageContainer narrow>
              <SpotlightHeading title="Families" seeAllHref="/families" seeAllLabel="See all families" />
              <div className="grid min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {families.map((family) => (
        ...
        {places.length > 0 && (
          <Section noPadding className="min-w-0 overflow-x-hidden pb-12 md:pb-16">
            <PageContainer narrow>
              <SpotlightHeading title="Places" seeAllHref="/tree/places" seeAllLabel="See all places" />
              <div className="grid min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {places.map((place) => (
