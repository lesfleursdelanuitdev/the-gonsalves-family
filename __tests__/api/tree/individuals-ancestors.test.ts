...
describe("GET /api/tree/individuals/[xref]/ancestors", () => {
  it("returns 503 when DATABASE_URL is not set", async () => {
    ...
    try {
      const req = new NextRequest("http://localhost/api/tree/individuals/I1/ancestors");
  ...
  it("returns 404 for unknown xref", async () => {
    ...
    const req = new NextRequest("http://localhost/api/tree/individuals/UNKNOWN_XREF_99999/ancestors");
